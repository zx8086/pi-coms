...
In single-token mode there is no per-peer authorization: any token holder can prompt any agent. Directory mode (above) adds identity, name binding, per-principal revocation, and session ownership: only the principal that registered a session may open its SSE stream, heartbeat for it, delete it, send as it, or answer messages delivered to it (root and `*` principals act on every session). Who may prompt whom, and inbox reads, remain open to every authenticated principal by design: the shared inbox is the same history for every operator, and an agent's inbox is its conversation history (every prompt it was asked, by whom, and its reply) for 14 days (`PI_COMS_NET_HISTORY_RETAIN_MS`). Prompt and response bodies of every conversation therefore sit on the hub's mailbox volume for that window. Genuinely separate trust domains still belong on separate hubs. Request bodies above `PI_COMS_NET_MAX_BODY_BYTES` (default 1 MiB) are rejected with 413, and project names are restricted to plain directory names.
