...
| `coms_net_inbox` | `name?`, `limit?`, `since?`, `msg_id?` | Read a durable inbox non-destructively: retained mailbox messages, identical for every reader. `name` defaults to the shared duty inbox `ops` (`PI_COMS_NET_INBOX_NAME`). Listing bodies are 2000-char previews; `msg_id` returns one message in full (see [Monitoring](monitoring.md#the-durable-inbox-read-many-on-demand)) |
